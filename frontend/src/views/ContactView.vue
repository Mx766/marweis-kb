    ...
    <div class="contact-grid" v-loading="loading">
      <div v-for="card in filteredContacts" :key="card.id" class="contact-card" @click="openDetail(card)">
        <div class="card-header">
          <span class="org-icon"><el-icon :size="28"><OfficeBuilding /></el-icon></span>
          <div class="org-info">
            <h3>{{ card.title }}</h3>
        ...
        <div class="card-body">
          ...
          <p v-if="card.source" class="org-dept"><el-icon :size="14"><Document /></el-icon> {{ card.source }}</p>
        </div>
...
onMounted(async () => {
  loading.value = true
  try {
    const catId = (route.params.id as string) || '2855b1e8-2751-4cfb-bb17-32a73634e51c'
