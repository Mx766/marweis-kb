...
async function loadData() {
  try {
    const raw = await get('/api/device-reg/knowledge-graph')
  ...
  totalDocs.value = raw.totalDocs || nodes.reduce((s: number, n: any) => s + (n.value || 0), 0)
  catCount.value = raw.catCount || nodes.filter((n: any) => n.category <= 2 && n.value >= 0).length

  // Build hierarchical tree from flat nodes+links
  const childrenMap: Record<string, any[]> = {}
  const nodeMap: Record<string, any> = {}

  for (const n of nodes) {
    if (n.category > 2) continue // skip type nodes for tree
    nodeMap[n.id] = {
      name: n.label || n.name,
      fullName: n.name,
      value: n.value || 0,
  ...
  for (const l of links) {
    ...
    // Skip 临床研究部ss-reference dashed lines
    if (l.lineStyle?.type === 'dashed') continue
  ...
  for (const n of nodes) {
    if (n.category <= 2 && !hasParent.has(n.id) && nodeMap[n.id]?.children?.length > 0) {
...
onMounted(async () => {
  ...
  if (chartEl.value) {
    ...
    chart.on('click', (params: any) => {
      if (params.data?.id && !params.data.id.startsWith('type_') && !params.data.id.startsWith('root_')) {
