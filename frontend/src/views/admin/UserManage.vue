    ...
    <el-dialog v-model="dialogVisible" :title="editMode === 'add' ? '添加用户' : '编辑用户'" width="500px">
      <el-form :model="userForm" label-width="100px">
        ...
        <el-form-item label="密码" v-if="editMode === 'add'">
          <el-input v-model="userForm.password" type="password" placeholder="默认密码 ChangeMe123" show-password />
        </el-form-item>
...
const departments = computed(() =>
  auth.isSuperAdmin ? DEPARTMENTS : (auth.user?.department ? [auth.user.department] : [])
)
...
const userForm = ref({
  username: '', password: 'ChangeMe123', display_name: '', employee_id: '',
...
async function handleSaveUser() {
  try {
    if (editMode.value === 'add') {
      await post('/api/admin/users', { ...userForm.value, password: userForm.value.password || 'ChangeMe123' })
