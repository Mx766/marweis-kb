  ...
  <div class="admin-page">
    ...
    <el-table :data="categories" stripe row-key="id" default-expand-all v-loading="loading">
        ...
        <template #default="{ row }">
          <span :style="{ paddingLeft: row._level * 24 + 'px' }">
            ...
            <span style="color: var(--color-text-secondary); font-size: 12px; margin-left: 8px">
              ({{ row.document_count || 0 }} 篇文档)
            </span>
          </span>
        </template>
      </el-table-column>
      <el-table-column label="可见部门" width="350">
        <template #default="{ row }">
      ...
      <el-table-column label="描述" min-width="200">
        <template #default="{ row }">
          <span style="font-size:12px; color:var(--color-text-secondary)">{{ row.description || '-' }}</span>
        </template>
      </el-table-column>
...
const form = ref({
  name: '', parent_id: '', icon: '', visible_departments: [] as string[], description: '',
})
...
function resetForm() {
  form.value = { name: '', parent_id: '', icon: '', visible_departments: [], description: '' }
}
