...
def _build_tree(
    ...
    for cat in children:
        child_nodes = _build_tree(categories, doc_counts, cat.id)
        node = CategoryNode(
            id=str(cat.id),
            name=cat.name,
            ...
            allow_upload=cat.allow_upload,
            permission_mode=cat.permission_mode,
            children=child_nodes,
            document_count=doc_counts.get(str(cat.id), 0) + sum(
                ch.document_count for ch in child_nodes
            ),
        )
        nodes.append(node)
    return nodes
...
async def list_categories(
    ...
    count_rows = (await db.execute(
        ...
        .where(Document.is_deleted == False, Document.category_id.is_not(None))
        .group_by(Document.category_id)
    )).all()
    doc_counts = {str(cid): cnt for cid, cnt in count_rows}
    ...
    if current_user is None:
        ...
        return _build_tree(result.scalars().all(), doc_counts)
    ...
    visible = [
        ...
        and not (c.permission_mode == "admin_only" and current_user.role == "employee")
    ]
    return _build_tree(visible, doc_counts)
